 ...
 *
 * Usage) node import-role.js <indir>
 *
 * Example: node import-role.js iam/roles
 */
'use strict';
...
function readRoleFilePromise(filePath) {
  return FileUtil.readFilePromise(filePath)
  .then(text => JSON.parse(text) );
}

function createRolePromise(role, varSet) {
...
function createInstanceProfilePromise(roleName) {
  const params = {
  ...
  .then(data => {
    console.log(`Created InstanceProfile: ${roleName}`);
...
function addRoleToInstanceProfilePromise(profileName, roleName) {
  const params = {
  ...
  .then(data => {
    console.log('Added InstanceProfile:' + profileName + ' to RoleName:' + roleName);
  })
  .catch(err => {
...
function attachRolePolicyPromise(params) {
  ...
  .catch(err => {
    if (err.code === 'NoSuchEntity') {
      return `${Status.NG} ${policyName} does not exist\n`;
    }
    throw err;
  });
}
...
function createRoleAndAttachPolicies(role, varSet) {
  const roleName = StringUtil.expandVars(role.Role.RoleName, varSet);
  return createRolePromise(role, varSet)
  .then(() => {
    if (!isEc2Role(role)) return;

    return createInstanceProfilePromise(roleName)
    .then(result => {
      if (result !== null) {
        return addRoleToInstanceProfilePromise(roleName, roleName);
      }
    });
  })
  .then(() => {
...
function main(inDir, varSet) {
  ...
  .then(fileList => {
    return promisedLife([
      ...
      promisedStream(file => readRoleFilePromise(file) ),
      promisedStream(policy => createRoleAndAttachPolicies(policy, varSet) ),
      promisedStream(item => attachRolePolicyPromise(item) ),
      process.stdout
    ])
  })
  .catch(err => {
...
if (process.argv.length !== 3) {
  console.log('Usage) node import-role.js <indir>');
  process.exit(1);
} else {
  const envs = {
